...
public:
  ...
  void fetch(const ::envoy::api::v2::core::HttpUri& uri, const std::string& client_id,
             const std::string& client_secret, const std::string& code,
             const std::string& redirect_uri, JwtReceiver& receiver) override {
    ENVOY_LOG(trace, "{}", __func__);
    receiver_ = &receiver;
    auto body = fmt::format(
        "code={}&client_id={}&client_secret={}&redirect_uri={}&grant_type=authorization_code", code,
        client_id, client_secret, Http::Utility::urlSafeEncode(redirect_uri));
    fetcher_->fetch(uri, Http::Headers::get().MethodValues.Post, "", content_type_form, body,
                    *this);
  }
  ...
  void onFetchFailure(Common::Failure reason) override {
    ENVOY_LOG(debug, "{}: fetch jwt: error {}", __func__, enumToInt(reason));
